...
/// Initial prompt injected as fake "previous context" to bias the decoder
/// toward expected vocabulary (e.g., "Pulse" instead of "pulse" or "polls").
const INITIAL_PROMPT: &str = "Hey Pulse.";
...
pub enum PulseModel {
    ...
    Large,
    /// large-v3-turbo — near large-v3 quality, 6x faster (~1.6GB).
    Turbo,
...
impl PulseModel {
    ...
    /// All available model tiers for UI iteration.
    pub fn all() -> &'static [PulseModel] {
        &[
            PulseModel::Fast,
            PulseModel::Balanced,
            PulseModel::Quality,
...
impl std::fmt::Display for PulseModel {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Turbo => write!(f, "turbo (large-v3-turbo)"),
...
impl PulseEngine {
    ...
    pub fn new(model_tier: PulseModel) -> Result<Self> {
        ...
        // Load model weights in f32. Candle-transformers' Whisper encoder computes
        // positional embeddings internally in f32, so mixed-precision (f16 weights +
        // f32 internals) causes dtype mismatches. F32 is required for correctness.
        let vb = unsafe {
        ...
        let multilingual = matches!(
            model_tier,
            PulseModel::Large | PulseModel::Turbo | PulseModel::DistilLarge
        );
    ...
    pub fn transcribe_sync(
        ...
        let Some(trimmed) = preprocess_audio(pcm, sample_rate, channels) else {
        ...
        // Build prompt tokens with initial prompt to bias decoder vocabulary.
        let prompt_tokens = build_prompt_tokens(
            &self.tokenizer,
            sot_token,
            transcribe_token,
            timestamp_begin,
        ...
        // Also build a plain header (no initial prompt) for subsequent seek segments.
        let plain_prompt_tokens: Vec<u32> = if self.multilingual {
        ...
        if content_frames_from_pcm <= m::N_FRAMES {
            ...
            let result_tokens = decode_with_fallback(
                &mut self.model,
                &encoder_output,
                &prompt_tokens,
                eot_token,
            ...
            while seek < content_frames_from_pcm {
                ...
                // Use initial prompt only for the first segment; subsequent
                // segments use the plain header to avoid prompt leakage.
                let segment_prompt = if is_first_segment {
                    &prompt_tokens
                } else {
                    &plain_prompt_tokens
                };
...
/// Run greedy decode with temperature fallback on degenerate output.
fn decode_with_fallback(
    ...
    // All temperatures produced degenerate output; return the last attempt.
    greedy_decode(
        model,
        encoder_output,
        prompt_tokens,
        ...
        suppress,
        device,
        *TEMPERATURE_SCHEDULE.last().unwrap(),
    )
}
...
/// Build decoder prompt tokens with an initial prompt to bias vocabulary.
///
/// Produces: `[SOT, ...prompt_tokens, SOT, (en,) transcribe, <|0.00|>]`
///
/// The initial prompt acts as fake "previous context" — Whisper treats it as
/// if the user already said those words, biasing the decoder toward that vocabulary.
/// Falls back to a plain header if tokenization fails.
fn build_prompt_tokens(
    ...
    let base_header: Vec<u32> = if multilingual {
        let en_token = tokenizer
            .token_to_id("<|en|>")
            .unwrap_or_else(|| panic!("Missing token: <|en|>"));
        vec![sot_token, en_token, transcribe_token, timestamp_begin]
    } else {
        vec![sot_token, transcribe_token, timestamp_begin]
    };

    // Tokenize the initial prompt; fall back to plain header on failure.
    let encoded = match tokenizer.encode(INITIAL_PROMPT, false) {
        Ok(enc) => enc,
        Err(e) => {
            warn!("Failed to tokenize initial prompt, skipping: {}", e);
            return base_header;
        }
    };
    ...
    // [SOT, ...prompt_tokens, SOT, (en,) transcribe, <|0.00|>]
    ...
    if multilingual {
        let en_token = tokenizer
            .token_to_id("<|en|>")
            .unwrap_or_else(|| panic!("Missing token: <|en|>"));
        tokens.extend_from_slice(&[sot_token, en_token, transcribe_token, timestamp_begin]);
    } else {
        tokens.extend_from_slice(&[sot_token, transcribe_token, timestamp_begin]);
    }

    debug!("Initial prompt tokens ({} total): {:?}", tokens.len(), tokens);
    tokens
}
...
mod tests {
    ...
    fn test_pulse_model_parse() {
        ...
        assert_eq!(PulseModel::parse("turbo"), Some(PulseModel::Turbo));
        assert_eq!(PulseModel::parse("large-v3-turbo"), Some(PulseModel::Turbo));
