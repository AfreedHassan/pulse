...
impl AudioCapture {
    pub fn new() -> Self {
        ...
        let supported_config = {
            ...
            // Sort by min sample rate so we can pick the lowest range that covers 16kHz+.
            configs.sort_by_key(|c| c.min_sample_rate());
            // Find a config whose range includes a rate >= 16kHz, pick the lowest such rate.
            // Fall back to the highest available rate if nothing covers 16kHz.
    ...
    pub fn drain(&self, count: usize) -> Vec<f32> {
        ...
        let drained: Vec<f32> = live[..n].to_vec();
        live.drain(..n);
        drained
    }
